...
void concentric(proc_t const& proc) {
  for (int ix = 0; ix < N; ++ix) {
    int cx = RECTANGULAR_WIDTH / 2;
    int cy = RECTANGULAR_HEIGHT / 2;
    int cz = RECTANGULAR_DEPTH / 2;
    sudare_clear();
    int power = get_power(ix);
    for (int x = 0; x < RECTANGULAR_WIDTH; ++x) {
      for (int y = 0; y < RECTANGULAR_HEIGHT; ++y) {
        for (int z = 0; z < RECTANGULAR_DEPTH; ++z) {
...
bool can_show(xyz_t const& p) {
  return 0 <= p.x && p.x < RECTANGULAR_WIDTH && 0 <= p.y &&
         p.y < RECTANGULAR_HEIGHT && 0 <= p.z && p.z < RECTANGULAR_DEPTH;
}
...
void color_cube() {
  ...
  for (int i = 0; i < N; ++i) {
    ...
    for (auto p0 : points) {
      ...
      double x = (p.x / 2 + 0.5) * (RECTANGULAR_WIDTH - 1);
      double y = (p.y / 2 + 0.5) * (RECTANGULAR_HEIGHT - 1);
      double z = (p.z / 2 + 0.5) * (RECTANGULAR_DEPTH - 1);
      if (can_show({x, y, z})) {
        sudare_set_led_rect(x, y, z, p0.color);
      }
    }
    sudare_send();
    sudare_sleep(50);
...
void fireworks() {
  ...
  for (int ix = 0; ix < N; ++ix) {
    sudare_clear();
    if (ix % 20 == 0 && 20 < N - ix) {
      double cx = RECTANGULAR_WIDTH * dist(rng);
      double cy = RECTANGULAR_HEIGHT * dist(rng);
      double cz = RECTANGULAR_DEPTH * dist(rng);
      for (int i = 0; i < PS; ++i) {
    ...
    for (size_t i = 0; i < poss.size(); ++i) {
      ...
      auto& v = vs[i];
      if (can_show(p.p)) {
        sudare_set_led_rect(p.p.x, p.p.y, p.p.z, p.color);
      } else {
      }
      p.p = p.p + v;
...
void repbang() {
  ...
  for (int i = 0; i < N; ++i) {
    ...
    for (auto& p : ps) {
      ...
      xyz_t pos =
          q * r * RECTANGULAR_HEIGHT +
          xyz(RECTANGULAR_WIDTH, RECTANGULAR_HEIGHT, RECTANGULAR_DEPTH) * 0.5;

      if (can_show(pos)) {
        sudare_set_led_rect(pos.x, pos.y, pos.z, p.color);
      }
    }
    sudare_send();
    sudare_sleep(50);
...
void spiral() {
  ...
  for (int i = 0; i < N; ++i) {
    ...
    for (auto& pc : pcs) {
      double y =
          int((pc.p.y) * RECTANGULAR_HEIGHT + N - i * U) % RECTANGULAR_HEIGHT;
      double x = (pc.p.r * std::cos(pc.p.t + i * T)) * RECTANGULAR_WIDTH / 2 +
                 RECTANGULAR_WIDTH / 2;
      double z = (pc.p.r * std::sin(pc.p.t + i * T)) * RECTANGULAR_WIDTH / 2 +
                 RECTANGULAR_DEPTH / 2;
      if (can_show({x, y, z})) {
        sudare_set_led_rect(x, y, z, darken(pc.c, d));
      }
    }
    sudare_send();
    sudare_sleep(50);
...
void brown() {
  ...
  std::deque<pos_t> poses[3]{
      {{RECTANGULAR_WIDTH / 2, RECTANGULAR_HEIGHT / 4, RECTANGULAR_DEPTH / 2}},
      {{RECTANGULAR_WIDTH / 2, RECTANGULAR_HEIGHT / 2, RECTANGULAR_DEPTH / 2}},
      {{RECTANGULAR_WIDTH / 2, RECTANGULAR_HEIGHT * 3 / 4,
        RECTANGULAR_DEPTH / 2}}};

  auto normalize = [](pos_t const& s) -> pos_t {
    return {(s.x + RECTANGULAR_WIDTH) % RECTANGULAR_WIDTH,
            (s.y + RECTANGULAR_HEIGHT) % RECTANGULAR_HEIGHT,
            (s.z + RECTANGULAR_DEPTH) % RECTANGULAR_DEPTH};
  };
  ...
  for (int i = 0; i < len; ++i) {
    ...
    for (int i = 0; i < 3; ++i) {
      ...
      for (auto it = pos.begin(); it != pos.end(); ++it, ++ix) {
        ...
        int rgb = (r << 16) + (g << 8) + b;
        sudare_set_led_rect(it->x, it->y, it->z, rgb);
      }
    }
    sudare_send();
    sudare_sleep(50);
...
void gala() {
  ...
  for (int i = 0; i < N; ++i) {
    ...
    for (auto& star : stars) {
      ...
      xyz_t pos{p.x * RECTANGULAR_WIDTH / 2 + RECTANGULAR_WIDTH / 2,
                p.y * RECTANGULAR_WIDTH / 2 + RECTANGULAR_HEIGHT / 2,
                p.z * RECTANGULAR_WIDTH / 2 + RECTANGULAR_DEPTH / 2};
      if (can_show(pos)) {
        sudare_set_led_rect(pos.x, pos.y, pos.z, darken(star.c, d));
      }
    }
    sudare_send();
    sudare_sleep(50);
...
void balls() {
  ...
  for (int i = 1; i < 7; ++i) {
    ps.push_back(
        {{static_cast<double>(i & 1 ? 0 : RECTANGULAR_WIDTH),
          static_cast<double>(i & 2 ? 0 : RECTANGULAR_HEIGHT),
          static_cast<double>(i & 4 ? 0 : RECTANGULAR_DEPTH)},
         {0, 0, 0},
  ...
  for (int ix = 0;; ++ix) {
    ...
    for (auto& p : ps) {
      if (ix < N && (!can_show(p.p) || p.v.len() == 0)) {
        xyz_t dir;
        for (;;) {
          xyz_t dest{static_cast<double>(rng() % RECTANGULAR_WIDTH),
                     static_cast<double>(rng() % RECTANGULAR_HEIGHT),
                     static_cast<double>(rng() % RECTANGULAR_DEPTH)};
          dir = dest - p.p;
          if (dir.len() != 0) {
            break;
          }
      ...
      for (int i = 0; i < 125; ++i) {
        ...
        if (delta.len() < 2.5) {
          ...
          if (can_show(pos)) {
            shown = true;
            sudare_set_led_rect(pos.x, pos.y, pos.z, p.c);
          }
        }
      }
    }
...
void threed() {
  ...
  std::vector<pp_t> pps;
  while (pps.size() < 2000) {
    xyz_t a{static_cast<double>(rng() % RECTANGULAR_WIDTH),
            static_cast<double>(rng() % RECTANGULAR_HEIGHT),
            static_cast<double>(rng() % RECTANGULAR_DEPTH)};
    xyz_t b{static_cast<double>(rng() % RECTANGULAR_WIDTH),
            static_cast<double>(rng() % RECTANGULAR_HEIGHT),
            static_cast<double>(rng() % RECTANGULAR_DEPTH)};
  ...
  auto show = [&col_at2, &pps](double c, double d) -> void {
    ...
    for (auto& pp : pps) {
      ...
      if (can_show(p)) {
        sudare_set_led_rect(p.x, p.y, p.z, col);
      }
    }
    sudare_send();
    sudare_sleep(50);
...
void wave() {
  ...
  for (int i = 0; i < N0; ++i) {
    ...
    for (int iy = 0; iy < RECTANGULAR_HEIGHT; ++iy) {
      for (int ix = 0; ix < RECTANGULAR_WIDTH; ++ix) {
        double x = xt * (ix - RECTANGULAR_WIDTH / 2) / RECTANGULAR_WIDTH * 2;
        double y = yt * (iy - RECTANGULAR_HEIGHT / 2) / RECTANGULAR_HEIGHT * 2;
        double iz = static_cast<int>(sin(x) * sin(y) * sin(i * it) *
                                         RECTANGULAR_DEPTH / 2 +
                                     RECTANGULAR_DEPTH / 2);
        auto col = [&](double t) -> int {
          double dx = (ix - RECTANGULAR_WIDTH / 2);
          double dy = (iy - RECTANGULAR_HEIGHT / 2);
        ...
        for (int dz : {-1, 0, 1}) {
          if (can_show({static_cast<double>(ix), static_cast<double>(iy),
                        iz + dz})) {
            sudare_set_led_rect(ix, iy, iz + dz, c);
          }
        }
      }
    }
